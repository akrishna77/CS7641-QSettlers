 ...
 * @since 1.1.00
 */
/*package*/ class SOCLocal3PClient implements Runnable
{
    /**
     ...
     * @since 1.1.13
     */
    public static Hashtable<String, Sample3PClient> robotClients = new Hashtable<String, Sample3PClient>();

    /**
     * This bot's client in {@link #robotClients}.
     */
    final Sample3PClient rob;

    protected SOCLocal3PClient(Sample3PClient rc)
    {
        rob = rc;
    }
    ...
    public void run()
    {
        ...
        Thread.currentThread().setName("local3probotclient-" + rname);  // was robotrunner- in v1.x.xx
        ...
        throws ClassNotFoundException, LinkageError
    {
        ...
        if (strSocketName != null)
            rcli = new Sample3PClient(strSocketName, rname, "pw", cookie);
        else
            rcli = new Sample3PClient("localhost", port, rname, "pw", cookie);
        rcli.printedInitialWelcome = true;  // don't clutter the server console

        Thread rth = new Thread(new SOCLocal3PClient(rcli));
        rth.setDaemon(true);
